...
<template>
  ...
  >
    ...
    <div class="ml-2 rtl:mr-2 min-w-0 rtl:ml-0">
      ...
      >
        <span v-if="isAdmin && email" class="truncate text-n-slate-12" :title="email">
          {{ email }}
        </span>
        <span v-if="isAdmin && phone" class="text-n-slate-10">•</span>
        <span v-if="isAdmin && phone" class="text-n-slate-12">{{ phone }}</span>
      </p>
    </div>
  </router-link>
</template>
